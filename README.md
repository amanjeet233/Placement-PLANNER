...
flowchart TD
    ...
    subgraph Firebase["🔥 Google Firebase Cloud (₹0 Spark Tier)"]
        Firestore[("Firestore NoSQL DB\n(placement_prep_sync)")]
        FCM["Firebase Cloud Messaging\n(HTTP v1 API)"]
    end

    subgraph GitHub["⚙️ GitHub Actions Automation"]
        Cron["Scheduled Cron Trigger\n(*/15 * * * *)"]
        Sender["FCM Push Sender\n(scripts/send-fcm-reminders.js)"]
        Secrets[("Encrypted Repository Secrets\n(RS256 Private Key, Email, Tokens)")]
    end

    UI <--> State
    State <--> CloudClient
    CloudClient <-->|Real-time Snapshot| Firestore

    Cron --> Sender
    Secrets -.-> Sender
    Sender -->|OAuth2 RS256 JWT Token| FCM
    FCM -->|Push Notification Event| FCM_SW
    FCM_SW -->|OS Banner Display| UI
    SW -->|Offline Cache Handling| UI
```
